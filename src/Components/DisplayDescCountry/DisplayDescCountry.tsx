...
const DisplayDescCountry: React.FC<Props> = ({ alpha3Code }) => {
  ...
  useEffect(() => {
    const getPostByCode = async () => {
      const response: { data: ICountry } = await axios<ICountry>(
        urlGetOnePost + alpha3Code,
      );
      ...
      if (responseCount.borders && responseCount.borders.length) {
        bordersName = await Promise.all(
          responseCount.borders.map(async (code) => {
            try {
              const responseBorder = await axios<ICountry>(
                urlGetOnePost + code,
              );
      ...
      <div className="mt-4 border border-gray p-4 bg-secondary-subtle">
        <div className="row p-5">
          <div className="col-4">
            <h3>{post.name}</h3>
            <strong>
              Capital: <p>{post.capital}</p>
            </strong>
            <strong>
              Population: <p>{post.population}</p>
            </strong>
          </div>
          <div className="col-7">
